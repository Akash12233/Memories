...
const Media: React.FC = () => {
  const settings =localStorage.getItem("settings") ? JSON.parse(localStorage.getItem("settings")) : null;
    ...
    useEffect(() => {
      axios.get(`/api/media/getimage/${id1}`)
      .then((response) => {
    ...
    useEffect(() => {
      axios.get(`/api/media/getvideo/${id1}`)
      .then((response) => {
    ...
    const handleFilterClick = (type) => {
      ...
      const handleSubmit = (e) => {
        e.preventDefault();
        console.log(formdata.images[0]?.size/ (1024*1024) );
        if(formdata.images){
          axios.post(`/api/media/addimage/${id1}`, formdata.images
          ,{
            headers: {
          ...
          .then((response) => {
            ...
            setError("Media uploaded successfully");
          })
          .catch((error) => {
            console.error(error);
      ...
      if(formdata.videos){
        axios.post(`/api/media/addvideo/${id1}`, formdata.videos,{
          headers: {
            'Content-Type': 'multipart/form-data',
          }
        })
